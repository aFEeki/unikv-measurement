...
- **Deterministic**: token sequences, spill counts, passkey outcomes and graph
  split counts. Greedy decoding at a fixed seed, so these reproduce on any
...
| Table 6; Fig. 3 is drawn from the sweep block below | `run_drain_control.py`, `make_fig7_drain.py` | `alpha_results/p3_drain_control_master.csv` |
| Table 7: drain×α interaction | `run_drain_alpha1.py` | `alpha_results/p3_drain_alpha1_master.csv` |
| Table 8, Fig. 4: scratch term, ceilings, ubatch linearity, prefill non-effect | `run_f4_phaseA.py`, `make_fig6_capacity.py` | `stress_results/f4_a1_ubatch_sweep.csv` |
| Table 9: upstream completes the workload; the exact-retention arm's device and host footprints | `run_capacity_workload.py`, `run_capacity_demo.py` | `stress_results/f4_a2_continuation_arms.csv`, `f4_a3_flash_attn_capacity.csv`, `capacity_demo.csv` |
...
| Cross-model tests (Qwen2.5 7B, Llama 3.2 3B): Section 4 coefficients, Section 7 scratch prediction, Section 5 replication | `run_b2_cooled.py`, `run_f4_phaseA.py`, `run_token_horizon.py` | `stress_results/qwen_isochronal_both_modes.csv`, `l3b_isochronal_both_modes.csv`, `f4_a1_ubatch_sweep_qwen.csv`, `f4_a1_ubatch_sweep_l3b.csv`, `quality_results/token_horizon_qwen.csv`, `token_horizon_l3b.csv` |
| Table 2: the out-of-sample predictor test, and the fourth model (Llama 3.2 1B) | `run_b2_cooled.py`, `run_f4_phaseA.py`, `run_token_horizon.py`, `analyze_isochronal.py` | `stress_results/l1b_isochronal_both_modes.csv`, `f4_a1_ubatch_sweep_l1b.csv`, `quality_results/token_horizon_l1b.csv` |
| Table 3: γ measured at the plateau; the affine form below 512 cells | `run_b2_cooled.py` with small targets, `analyze_gamma_small.py` | `stress_results/llamasmall_isochronal_both_modes.csv`, `qwensmall_isochronal_both_modes.csv`, `l3bsmall_isochronal_both_modes.csv` |
...
| Drain cost across the range: saturation and curvature (Table 7, block 3) | `run_drain_sweep.py` | `alpha_results/p3_drain_sweep_master.csv` |
| Table 11: discard granularity, shift-event census, and the second per-event block | `run_amortized_window.py`, `analyze_reconstruction.py`, `check_slot_effects.py` | `stress_results/amortized_window_block.csv`, `amortized_window_events.csv`, `amortized_window_recheck.csv`, `amortized_window_recheck_c2048.csv` |
...
| Table 10: policy comparison, exactness premium | `run_b2_cooled.py` | `stress_results/b2_policy_block.csv` |
